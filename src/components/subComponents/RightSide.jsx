...
const RightSide = () => {
    const cards = [
        {
            ...
            tasks: [
                ...
                { taskName: "Update docs", startedAt: "02:00 PM", status: "ongoing" },
        ...
        },
        {
            ...
            userStatus: "active",
            tasks: [
                { taskName: "Follow up", startedAt: "11:00 AM", status: "completed" },
                { taskName: "Lunch Break", startedAt: "12:00 PM", status: "break" },
    ...
    // Build timeline using current time as the end of the last task
    const buildTimeline = (loginAt, tasks) => {
        ...
        const now = new Date();
        const logout = now > officeEnd ? officeEnd : now;
        const totalMinutes = (officeEnd - officeStart) / 60000;

        let timeline = [];
        let lastEnd = officeStart;

        // Sort tasks by start time
        const sortedTasks = tasks.map(task => ({
            ...task,
            startDate: parseTime(task.startedAt)
        ...
        sortedTasks.forEach((task, idx) => {
            ...
            const taskEnd = (idx < sortedTasks.length - 1)
                ? sortedTasks[idx + 1].startDate
                : logout; // last task ends at current time
    ...
    const TimelineBar = ({ loginAt, tasks }) => {
        const segments = buildTimeline(loginAt, tasks);
                ...
                {segments.map((seg, idx) => (
                    ...
                    >
                        ...
                        <div
                            ...
                            pointer-events-none bg-black text-white text-[12px] px-1 py-0.5 rounded
                        ...
                        >
                            ...
                            {idx === 0 && loginDeviationText && <div className="text-white">{loginDeviationText}</div>}
                        </div>
                    </div>
                ))}
            </div>
    ...
    const getLateMinutes = (loginAt) => {
        const officeStart = new Date();
        officeStart.setHours(10, 0, 0, 0);

        const loginTime = loginAt ? new Date(loginAt) : officeStart;
        const diff = (loginTime - officeStart) / 60000; // difference in minutes
        ...
        <div className="md:w-1/2 w-full flex flex-col">
            ...
            {cards.map((item) => (
                <div key={item.id} className={`w-full p-3 border-b ${item.userStatus === "loggedOut" ? "opacity-50" : ""}`}>
                    ...
                    <TimelineBar loginAt={item.loginAt} tasks={item.tasks} />
